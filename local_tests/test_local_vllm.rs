...
const HOST: &str = "http://localhost:30002/v1";
...
fn main() {
    if !server_reachable() {
        eprintln!("Skipping: vLLM server not reachable at localhost:{PORT}");
        return;
    }

    let backend = VLLMBackend::with_config(MODEL, HOST);
    ...
    // 9. Dataset
    println!("\n=== dataset ===");
    let csv_path = dataset_runner::run_dataset_and_save_csv(&classifier, "vllm", MODEL)
