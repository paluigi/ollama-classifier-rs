//! LOCAL-ONLY integration test against a real vLLM server.
//!
//! Standalone binary that connects to a running vLLM instance and exercises
...
//! 2. The model reachable at `http://localhost:30002/v1`.
//!
//! The program prints a message and exits early if vLLM is unreachable.
...
use ollama_classifier_rs::backends::{LLMBackend, VLLMBackend};
use ollama_classifier_rs::{ClassificationResult, LLMClassifier};

const MODEL: &str = "Qwen/Qwen2.5-3B-Instruct";
const HOST: &str = "localhost";
const PORT: u16 = 30002;
const BASE_URL: &str = "http://localhost:30002/v1";
...
fn dataset<B: LLMBackend>(classifier: &LLMClassifier<B>) -> bool {
    println!("\n[9/9] dataset");
    match dataset_runner::run_dataset_and_save_csv(classifier, "vllm", MODEL) {
        Ok(path) => {
...
fn main() {
    println!("=== Local integration test: vLLM ({MODEL}) ===\n");

    if !port_open(HOST, PORT) {
        eprintln!("vLLM server not reachable at {HOST}:{PORT} — skipping.");
        return;
    }

    let backend = VLLMBackend::with_config(MODEL, BASE_URL);
