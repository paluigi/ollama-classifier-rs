...
const HOST: &str = "http://localhost:30001/v1";
...
fn main() {
    if !server_reachable() {
        eprintln!("Skipping: llama.cpp server not reachable at localhost:{PORT}");
        return;
    }

    let backend = LlamaCppBackend::with_config(MODEL, HOST);
    ...
    // 9. Dataset
    println!("\n=== dataset ===");
    let csv_path = dataset_runner::run_dataset_and_save_csv(&classifier, "llamacpp", MODEL)
        .expect("dataset CSV save failed");
    println!("  CSV: {csv_path}");

    println!("\n✓ All llama.cpp tests passed!");
}
