//! LOCAL-ONLY integration test against a real llama.cpp server.
//!
//! Standalone binary that connects to a running `llama-server` instance and
//! exercises both scoring methods of [`LLMClassifier`]:
//!
...
//! Prerequisites
//! -------------
//! 1. llama.cpp server running on port 30001 with the GGUF model loaded:
//!    `llama-server --model qwen2.5-3b-instruct-q4_k_m.gguf --host 0.0.0.0 --port 30001`
//! 2. The model reachable at `http://localhost:30001/v1`.
//!
//! The program prints a message and exits early if the server is unreachable.
...
use ollama_classifier_rs::backends::{LLMBackend, LlamaCppBackend};
...
const BASE_URL: &str = "http://localhost:30001/v1";
...
fn dataset<B: LLMBackend>(classifier: &LLMClassifier<B>) -> bool {
    println!("\n[9/9] dataset");
    match dataset_runner::run_dataset_and_save_csv(classifier, "llamacpp", MODEL) {
        Ok(path) => {
...
fn main() {
    println!("=== Local integration test: llama.cpp ({MODEL}) ===\n");

    if !port_open(HOST, PORT) {
        eprintln!("llama.cpp server not reachable at {HOST}:{PORT} — skipping.");
        return;
    }

    let backend = LlamaCppBackend::with_config(MODEL, BASE_URL);
