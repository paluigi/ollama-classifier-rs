//! LOCAL-ONLY integration test against a real SGLang server.
//!
//! Standalone binary that connects to a running SGLang instance and exercises
...
//! Prerequisites
//! -------------
//! 1. SGLang server running on port 30000 with the GGUF model loaded.
//! 2. The model reachable at `http://localhost:30000/v1`.
//!
//! Note: SGLang's first request after a cold start can take ~15s while the
//! model loads, so the skip guard uses a generous timeout.
//!
//! The program prints a message and exits early if SGLang is unreachable.
...
use ollama_classifier_rs::backends::{LLMBackend, SGLangBackend};
...
const BASE_URL: &str = "http://localhost:30000/v1";
...
fn dataset<B: LLMBackend>(classifier: &LLMClassifier<B>) -> bool {
    println!("\n[9/9] dataset");
    match dataset_runner::run_dataset_and_save_csv(classifier, "sglang", MODEL) {
        Ok(path) => {
...
fn main() {
    println!("=== Local integration test: SGLang ({MODEL}) ===\n");

    if !port_open(HOST, PORT) {
        eprintln!("SGLang server not reachable at {HOST}:{PORT} — skipping.");
        return;
    }

    let backend = SGLangBackend::with_config(MODEL, BASE_URL);
